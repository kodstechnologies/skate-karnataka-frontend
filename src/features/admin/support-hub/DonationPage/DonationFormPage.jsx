...
import donationHero from "@/assets/Skating_header.jpg";
...
export const DonationFormPage = () => {
  ...
  const passedDonation = state?.donation;
  const isEditing = Boolean(id);

  const [formData, setFormData] = useState(
    passedDonation
      ? {
          brandName: passedDonation.brandName ?? "",
          title: passedDonation.title ?? "",
          about: passedDonation.about ?? "",
          contribution: passedDonation.contribution ?? "",
          duration: passedDonation.duration ?? "",
          donorName: passedDonation.donorName ?? "",
          amount: passedDonation.amount ?? "",
          img: null
        }
      : defaultForm
  );
  ...
  const [fetchError] = useState(
    isEditing && !passedDonation
      ? "Please access this page by clicking edit on a donation card."
      : null
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      payload.append("donorName", formData.donorName.trim());
      ...
      console.log(payload);
      if (isEditing) {
        await donationApi.update(id, payload);
      } else {
        await donationApi.create(payload);
      }

      navigate("/support-hub/donation");
    } catch (error) {
    ...
    return (
      <Paper elevation={0} sx={{ p: 4, borderRadius: "28px", textAlign: "center" }}>
        ...
        <Button
          sx={{ mt: 3 }}
          variant="contained"
          onClick={() => navigate("/support-hub/donation")}
        >
          Back to donations
        </Button>
      </Paper>
    ...
    <Box className="space-y-5">
      ...
      <Paper
        ...
        sx={{
          ...
          background: `linear-gradient(120deg, rgba(18, 14, 16, 0.82) 0%, rgba(38, 25, 26, 0.62) 34%, rgba(246, 118, 94, 0.2) 100%), url("${donationHero}")`,
      ...
      >
        <Stack sx={{ position: "relative", zIndex: 1 }}>
          ...
          <Typography sx={{ color: "rgba(255,255,255,0.86)", maxWidth: 700, lineHeight: 1.7 }}>
            {isEditing
              ? "Update the details or image of this donation."
      ...
      >
        <Stack spacing={2.5}>
            ...
            <Stack spacing={2}>
              ...
              >
                ...
                <TextField
                  ...
                  error={Boolean(formErrors.donorName)}
              ...
              <Box>
                ...
                <Typography sx={{ mt: 0.75, color: "#8d7f7b", lineHeight: 1.7 }}>
                  {isEditing
                    ...
                    : "Upload an image for this donation."}
                </Typography>
              </Box>
            </Stack>
          ...
          >
            ...
            >
              {submitting ? "Saving…" : isEditing ? "Save Changes" : "Create Donation"}
            </Button>
          </Stack>
        </Stack>
      </Paper>

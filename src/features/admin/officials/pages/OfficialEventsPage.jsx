...
export const OfficialEventsPage = () => {
  ...
  const [officialName, setOfficialName] = useState(officialFromStore?.fullName || "");
  ...
  const fetchEvents = useCallback(
    async (search = "", currentPage = 1, limit = 9) => {
      ...
      setError(null);
      try {
        const response = await stateApi.getOfficialEvents(officialId, {
          page: currentPage,
          limit,
        ...
        const message = err?.response?.data?.message || "Failed to fetch official events";
        toast.error(message);
        setError(message);
      } finally {
        setLoading(false);
  ...
  }, [officialId, page, rowsPerPage, fetchEvents]);
  ...
  const displayName = officialName || officialFromStore?.fullName || "Official";
        ...
        <Stack spacing={2} sx={{ position: "relative", zIndex: 1 }}>
          ...
          <Box>
            ...
            <Typography sx={{ color: "rgba(255,255,255,0.82)", lineHeight: 1.7, maxWidth: 620 }}>
              State-level events managed under this official account.
            </Typography>
          </Box>
        ...
        >
          <Box>
            ...
            <Typography sx={{ mt: 0.75, color: "#8d7f7b" }}>
              State events linked to this official.
            </Typography>
          </Box>
